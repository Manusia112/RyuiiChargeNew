...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  // ── Validate: check transaction exists + status transition is legitimate ──
  const { data: existingTx, error: fetchErr } = await client
    .from("transactions")
  ...
  if (fetchErr || !existingTx) {
    ...
    await logError(client, {
      ...
      errorMessage: fetchErr ? String(fetchErr.message) : `No transaction for ref_id=${refId}`,
      rawResponse:  payload,
    });
    return ack();
  }
  ...
  if (!(validTransitions[currentStatus] ?? []).includes(newStatus)) {
    console.warn(`[digiflazz-callback] Invalid transition ${currentStatus} → ${newStatus} for ${refId} — ignoring`);
    return ack();
  }
